...
function chooseAppToRun() {
  ...
  switch (app) {
    ...
    default:
      console.log(
        'Usage is my-tweets, spotify-this-song, movie-this, or do-what-it-says'
      );
  }
}
...
function getTweets() {
  ...
  client.get('statuses/user_timeline', { screen_name: 'Rob Wawrzak' }, function(
    error,
    tweets,
    response
  ) {
    ...
    function(err, data) {
      if (err) {
        return console.log(
          'Error occurred: ' +
            err +
            '. You might try searching for another song title.'
        );
      }
...
function getMovie(param) {
  ...
  request('http://www.omdbapi.com/?t=' + title + '&apikey=trilogy', function(
    error,
    response,
    body
  ) {
...
function doWhatItSays() {
  ...
  fs.readFile(textFile, 'utf8', function(error, data) {
    ...
    switch (app) {
      ...
      default:
        console.log(
          'Make sure the file contains the proper usage for my-tweets, spotify-this-song, or movie-this.'
        );
    }
  });
}
